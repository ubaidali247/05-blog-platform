// Search and Filter Tests - Blog Platform
describe('Search and Filter - Blog Platform', () => {
  beforeEach(() => {
    cy.visit('/');
  ...
  it('searching for existing item title shows result', () => {
    cy.get('#posts-list .item-card').first().find('.item-title').invoke('text').then(title => {
  ...
  it('searching for non-existent term shows empty state', () => {
    ...
    cy.get('#posts-list').should('contain', 'No items found');
  });
  ...
  it('clearing search restores full list', () => {
    cy.get('#posts-list .item-card').then($cards => {
      ...
      cy.get('#posts-list .item-card').should('have.length', total);
    });
  });
  ...
  it('filter by Technology category works', () => {
    cy.get('#filter-category').select('Technology');
  ...
  it('api search endpoint filters results', () => {
    cy.request('/api/posts?search=test').its('body').should('be.an', 'array');
  });

  it('api category filter endpoint works', () => {
    cy.request('/api/posts?category=Technology').its('body').should('be.an', 'array');
  });
  ...
  it('api returns all items when no filter applied', () => {
    cy.request('/api/posts').its('body').should('have.length.gte', 1);
  });
