// Navigation Tests - Blog Platform
describe('Navigation - Blog Platform', () => {
  beforeEach(() => {
    cy.visit('/');
  });
  ...
  it('loads the homepage successfully', () => {
    ...
    cy.get('.nav-brand').should('contain', 'Blog Platform');
  });
  ...
  it('navigates to posts list page', () => {
  ...
  it('cancel button returns to posts list', () => {
    cy.get('.nav-link[data-page="add"]').click();
    cy.get('#btn-cancel').click();
    cy.get('#page-posts').should('not.have.class', 'hidden');
  });

  it('shows add new button on posts list page', () => {
    cy.get('.nav-link[data-page="posts"]').click();
  ...
  it('api health endpoint returns correct project name', () => {
    cy.request('/api/health').its('body.project').should('eq', 'Blog Platform');
  });
});
